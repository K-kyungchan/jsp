...
//		}
		...
		// 로딩시 최초 1회 사용자가 요청을 할때마다 서버종료 or reload
		// init 메소드는 해당 서블릿에서 사용한 자원을 초기화 할 때 사용
		// 로딩시 최초1회 호출 : 로딩 되는 시점 ==> 해당서블릿으로 최초 요청이 들어왔을 때
		// 단 web.xml의 servlet 엘레멘트의 하위 엘레멘트인 load-on-startup엘레멘트 값으로
		// 양의 정수값을 입력할 경우 서버가 기동하면서 바로 init 메서드를 호출한다

		// servlet container가 요청을 처리하는 방법
		// 등록된 url 매핑을 참고하여 등록된 서블릿으로 요청을 전달(service 메서드 호출)
		// localhost/basicServlet ==> BasicServlet의 service메서드를 통해 응답생성
		// localhost/index.jsp ==> servet 설정에 있는 web.xml에 등록된
		...
		pw.println("<html>");
		pw.println("	<head>");
		pw.println("		<title>basci Servlet</title>");
		pw.println("	</head>");
		pw.println("	<body>");
		pw.println("		Hellow, World2" + new Date());
		pw.println("	</body>");
		pw.println("</html>");
